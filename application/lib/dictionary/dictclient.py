...
#import dictclient
#con = dictclient.Connection("dict.org")
#db = dictclient.Database(con, "!") #'*' - all result, '!' - only the first result
#defs = db.define(word)
#text = '<br/>'.join([item.getdefstr() for item in defs])
...
class Connection:
    ...
    def __init__(self, hostname = 'localhost', port = 2628):
        ...
        self.rfile = self.sock.makefile(encoding='utf-8')
        self.wfile = self.sock.makefile("wb", 0, encoding='utf-8')
